...
async def webhook(request: Request):
    ...
    try:
        ...
        if success: logger.info(f"Response sent to {user_name}") advisor_conversation_id = os.getenv("ADVISOR_CONVERSATION_ID") if advisor_conversation_id and "cruce de characato" in response.lower(): notification = ( f"📋 Nueva cita agendada\n" f"Cliente: {user_name}\n" f"Mensaje del cliente: {user_message}\n" f"Confirmación del bot: {response}" ) await provider.send_message(advisor_conversation_id, account_id, notification) logger.info("Advisor notified of new appointment") return JSONResponse({"status": "success"}, status_code=200) else: logger.error(f"Failed to send response to {user_name}") return JSONResponse({"status": "error"}, status_code=500)
